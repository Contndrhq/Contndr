    ...
    <div className="flex-1 min-h-0 space-y-2 -mx-1" aria-hidden="true">
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="flex items-center gap-2.5 py-2 px-2.5 rounded-lg">
          {withIcon && (
            <div className="w-7 h-7 rounded-lg bg-zinc-100 dark:bg-white/[0.06] skeleton-shimmer flex-shrink-0" />
          )}
          <div className="flex-1 min-w-0 space-y-1.5">
            <div
              className="h-3 rounded bg-zinc-100 dark:bg-white/[0.06] skeleton-shimmer"
              style={{ width: `${60 + ((i * 7) % 30)}%` }}
            />
            <div
              className="h-2.5 rounded bg-zinc-100 dark:bg-white/[0.04] skeleton-shimmer"
  ...
  return (
    <div className="flex-1 min-h-0 flex items-end gap-2 px-1 pb-1" aria-hidden="true">
      {heights.map((h, i) => (
        <div
          key={i}
          className="flex-1 rounded-t bg-zinc-100 dark:bg-white/[0.06] skeleton-shimmer"
          style={{ height: `${h}%` }}
        />
      ))}
    </div>
