...
export function CRM({ onStartFollowUp, onUpgrade }: { onStartFollowUp?: (leadIds: string[]) => void; onUpgrade?: () => void }) {
  ...
  async function loadTeamLeads() {
    try {
      const response = await authenticatedFetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8b2511f/lead-engine/team-leads?per_page=2000`,
        ...
        {loading || isDemoSeeding ? (
          ...
          <>
            ...
            <div className="block sm:hidden flex-1 min-h-0 overflow-y-auto p-4 space-y-4">{paginatedLeads.map((lead) => (<MobileLeadCard key={lead.id} lead={lead} selectedLeads={selectedLeads} toggleLeadSelection={toggleLeadSelection} setSelectedLeadId={setSelectedLeadId} statusColors={statusColors} needsReveal={needsReveal} revealLead={revealLead} revealingLeadId={revealingLeadId} showOutreach={teamLeads.length > 0} pipelineStage={pipelineLeadIds[lead.id] || null} intentLevel={highIntentLeadIds.has(lead.id) ? 'high' : mediumIntentLeadIds.has(lead.id) ? 'medium' : 'none'} onComposeEmail={handleComposeEmail} />))}</div><div className="hidden">
                  ...
                  <div className="flex justify-between items-start mb-3">
                     <div className="flex items-start gap-3 flex-1 overflow-hidden">
                        <input
                          ...
                          className="w-4 h-4 rounded border-zinc-300 dark:border-zinc-700 accent-[#1ED4A7] text-[#1ED4A7] focus:ring-[#1ED4A7] bg-white dark:bg-zinc-900 mt-1"
                        />
                        <div className="min-w-0 flex-1">
                           <div className="flex items-center gap-1.5">
                             <h3 className="font-semibold text-zinc-900 dark:text-white truncate">{toTitleCase(lead.business_name)}</h3>
                             {lead.is_shared && (
                               <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#1ED4A7]/10 text-[#1ED4A7] border border-[#1ED4A7]/20 flex-shrink-0">
                                 <Globe2 className="w-2.5 h-2.5" />
                                 Shared
                               </span>
                             )}
                           </div>
                           <p className="text-xs text-zinc-500 truncate">{lead.category || 'Uncategorized'}</p>
                     ...
                     <div className="flex flex-col items-end gap-1 ml-2">
                        {lead.stage && (
                          <span className={`px-2 py-0.5 rounded text-xs font-medium uppercase tracking-wide border ${
                             statusColors[lead.stage] || 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700'
                          }`}>
                            {lead.stage}
                          </span>
                        )}
                        ...
                        {(lead.emails_opened && lead.emails_opened > 0 || lead.emails_clicked && lead.emails_clicked > 0) && (
                           <div className="flex items-center gap-2 mt-1">
                             {lead.emails_opened && lead.emails_opened > 0 && (
                               <div className="flex items-center gap-1 text-[#1ED4A7]">
                                  <Eye className="w-3 h-3" />
                                  <span className="text-xs font-medium">{lead.emails_opened}</span>
                               </div>
                             )}
                             {lead.emails_clicked && lead.emails_clicked > 0 && (
                               <div className="flex items-center gap-1 text-[#1ED4A7]">
                                  <MousePointerClick className="w-3 h-3" />
                                  <span className="text-xs font-medium">{lead.emails_clicked}</span>
                               </div>
                             )}
                           </div>
                        )}
                  ...
                  <div className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400 mb-4 bg-zinc-50 dark:bg-zinc-950 p-3 rounded-lg">
                     {lead.contact_name ? (
                       <div className="flex items-center gap-2">
                         ...
                         <span className="truncate">{toTitleCase(lead.contact_name)}</span>
                       </div>
                     ) : (
                       <div className="flex items-center gap-2 text-zinc-400 italic">
                         <User className="w-4 h-4 flex-shrink-0" />
                         <span>{t('crm.unknownContact', 'Unknown Contact')}</span>
                       </div>
                     )}
                     ...
                     <>
                         {lead.email ? (
                           <div className="flex items-center gap-2">
                             ...
                             {needsReveal(lead) ? <span className="truncate text-zinc-400 select-none blur-[3px]">{maskEmail(lead.email)}</span> : <span className="truncate">{lead.email}</span>}
                           </div>
                         ) : (
                           <div className="flex items-center gap-2 text-zinc-400 italic">
                             <Mail className="w-4 h-4 flex-shrink-0" />
                             <span>{t('crm.noEmail', 'No Email')}</span>
                           </div>
                         )}
                           ...
                           <div className="flex items-center gap-2">
                             {needsReveal(lead) ? (
                               <>
                                 ...
                                 <span className="truncate text-zinc-400 select-none blur-[3px]">{maskPhone(lead.phone)}</span>
                               </>
                             ) : (
                               <SmartPhoneButton
                                 phone={lead.phone}
                                 ...
                                 numberClassName="text-xs text-zinc-700 dark:text-zinc-300 truncate"
                               />
                             )}
                           </div>
                         )}
              ...
              {/* END_HIDDEN_CARDS */}</div><div className="block sm:hidden px-4">{/* Pagination for Mobile */}
              {totalPages > 1 && (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-medium ${getAvatarColor(lead.contact_name || lead.business_name || '?')}`}>
                          {(lead.contact_name || lead.business_name || '?').charAt(0).toUpperCase()}
                        </div>
                        <div className="text-sm min-w-0">
                          <div className="flex items-center gap-1">
                            <p className="text-zinc-900 dark:text-white font-medium truncate max-w-[150px]">
                              {toTitleCase(lead.contact_name || lead.business_name) || t('crm.unknown', 'Unknown')}
                            </p>
                            {lead.is_shared && (
                              <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#1ED4A7]/10 text-[#1ED4A7] border border-[#1ED4A7]/20 flex-shrink-0">
                                <Globe2 className="w-2.5 h-2.5" />
                            ...
                            {lead.linkedin && (
                              <a 
                                ...
                                target="_blank" 
                                rel="noreferrer" 
                                className="text-[#1ED4A7] hover:text-[#1ED4A7]/80 flex-shrink-0"
                          ...
                          <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-[150px]">
                              {lead.business_name && lead.contact_name ? toTitleCase(lead.business_name) : (lead.website || lead.email || '')}
                            </p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm">
                        <p className="text-zinc-900 dark:text-white truncate max-w-[150px]">
                    ...
                    <td className="p-4" onClick={(e) => e.stopPropagation()}>
                      <div className="relative">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium cursor-pointer hover:opacity-80 transition-opacity whitespace-nowrap ${
                        ...
                        >
                          {updatingStatus === lead.id ? (
                            ...
                            <>
                              <span className="capitalize">{(lead.stage || lead.status || 'Cold').replace(/_/g, ' ')}</span>
                        ...
                        {statusMenuOpen === lead.id && (
                          <>
                            ...
                            />
                            <div className="absolute top-full left-0 mt-1 w-40 bg-white dark:bg-black rounded-lg shadow-xl border border-zinc-200 dark:border-zinc-800 z-20 overflow-hidden py-1">
                              {['Hot', 'Warm', 'Cold', 'Interested', 'Customer', 'Churned', 'Bad Fit', 'Do Not Contact'].map((status) => (
                                <button
                                  ...
                                  className={`w-full text-left px-3 py-2 text-xs hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors block ${
                                    (lead.stage || lead.status) === status ? 'bg-[#1ED4A7]/10 text-[#1ED4A7]' : 'text-zinc-700 dark:text-zinc-300'
                                  }`}
                                >
                                  {status}
                                </button>
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-center gap-4">
                         <div className={`flex items-center gap-1.5 ${lead.emails_opened && lead.emails_opened > 0 ? 'text-zinc-900 dark:text-white' : 'text-zinc-400 dark:text-zinc-600'}`} title={`${lead.emails_opened || 0} Emails Opened`}>
                         ...
                         <div className={`flex items-center gap-1.5 ${lead.emails_clicked && lead.emails_clicked > 0 ? 'text-zinc-900 dark:text-white' : 'text-zinc-400 dark:text-zinc-600'}`} title={`${lead.emails_clicked || 0} Links Clicked`}>
